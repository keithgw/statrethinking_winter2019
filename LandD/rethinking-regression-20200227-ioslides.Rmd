---
title: "Rethinking Linear Regression"
...
![](statisticalRethinkingCover.png)
...
```{r, echo=FALSE, messages=FALSE, warnings=FALSE, output = 'hide'}
library(tidyverse)
...
## An example with code {#io-example .reveal}
<div class="columns-2">
```{r, eval = FALSE}
...
adult_heights %>% 
  ...
  ggplot(aes(x = weight)) + 
  geom_smooth(
    aes(ymin = lower, ymax = upper, y = mu), 
    stat = "identity") + 
...
adult_heights %>% 
  ...
  geom_point(aes(y = height), alpha = 0.5) +
  theme_minimal() +
...
sampled_heights_df <- tibble(
  height = sampled_heights, 
  src = "sampled"
)
...
for (mu in all_mus) {
  for (sigma in all_sigmas) {
    ...
    posterior_probability <- bind_rows(posterior_probability, data.frame(mu, sigma, unscaled_probability))
  }
}

# scale probabilities such that sum(probability) = 1
...
```{r, cache=TRUE, output='hide', echo=FALSE, message=FALSE}
mdl <- brm(
...
ggplot() +
  ...
  geom_point(data = adult_heights, aes(weight, height), alpha = 0.4) +
  theme_minimal() +
  theme(text = element_text(size = 14)) +
  labs(title = "20 sampled regression lines \n w/ prediction interval", x = "weight (kg)", y = "height (cm)")
```
</div>
...
## Priors as regularization  
```{r, echo = FALSE, cache = TRUE, output='hide', message=FALSE}
mdl_r <- brm(
  formula = height ~ weight,
  data = adult_heights, 
  family = gaussian(),
...
ggplot() +
  ...
  geom_point(data = adult_heights, aes(weight, height), alpha = 0.5) +
  geom_abline(intercept = mean(posterior$b_Intercept), slope = mean(posterior$b_weight), color = "orange1", size = 2) + 
  theme_minimal() +
  theme(text = element_text(size = 14), title = element_text(size = 12)) +
  labs(
...
## Summarize  
1. Writing linear models as a likelihood, and modeling parameters as a linear model is a more general form that makes assumptions explicit.  
2. The target of Bayesian regression is a posterior probability on all estimated parameters.  
3. Posterior probabilites are learned as a product of the likelihood of your data and prior probabilities on the parameters of the likelihood.  
4. Priors can be used to encode your domain knowledge and to regularlize estimates.  
